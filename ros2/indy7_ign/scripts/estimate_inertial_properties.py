...
def main():
    ...
    # You can also use your own estimate of total mass if you managed to weigh Indy7 yourself :)
    total_mass = 18.0
    if len(sys.argv) > 1:
        if float(sys.argv[1]) > 0.0:
            total_mass = float(sys.argv[1])
        else:
            print("Error: Total mass of Indy7 (first argument) must be positive.")
            exit(1)
    ...
    visual_mesh_dir = path.join(path.dirname(path.dirname(
        path.realpath(__file__))), 'indy7', 'meshes', 'visual')
    ...
    output_file = 'indy7_inertial_out.sdf'
